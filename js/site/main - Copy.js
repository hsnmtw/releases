...
function stripEmptyOrNull(model){
    ...
    Object.keys(model).forEach(function(key){
        if(!isNullOrEmpty(model[key])) data[key] = model[key];
    });
    return data;
}
...
function DESERIALIZE(selector, data, properties) {
    ...
    if(typeof data === 'object' && (typeof properties !== 'object' || !Array.isArray(properties) || properties.length === 0)) properties = Object.keys(data);
    ...
    for (var i in properties) {
        ...
        if (Object.hasOwnProperty.call(model, key)) {
            ...
            ['name','id'].filter(function(prop){ return QSA(selector+' ['+prop+'="'+key+'"]').length > 0; }).take(1).forEach(function (prop) {
            //var prop = 'name';
...
function readFormData(data) {
    if (typeof (data) == "object" && false == Array.isArray(data)) return data;
    var result = {};
    for (var i in data) {
...
function ALERT(selector,r,onSuccess,onFailure){
    ...
    each(selector, function(alertbox){
        ...
        alertbox.innerHTML = ICONS[r.Status] + 
...
function setValue(element, value){
    ...
    else if (typeof el.attributes['data-toggle'] !== 'undefined') {
        if(value == null) el.value = null;
        else $(el).datepicker('setDate', toDate(value) );
    }
    else el.value = value;
}
